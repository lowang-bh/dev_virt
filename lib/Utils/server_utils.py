...
def print_server_hardware_info(**kwargs):
    ...
    log.info("\tCores per Socket: %s", (int(cpu_info.get("cpu_cores", 0)) / int(cpu_info.get("cpu_sockets", 1))))
    ...
    log.info("\tMemory total: %s MB", memory_info.get("size_total"))
    log.info("\tMemory used: %s MB", memory_info.get("size_used"))
    log.info("\tMemory free: %s MB", memory_info.get("size_free"))
