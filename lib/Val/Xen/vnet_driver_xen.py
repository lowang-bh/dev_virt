...
class XenVnetDriver(VnetDriver):
    ...
    def get_vif_by_index(self, inst_name, vif_index):
        ...
        log.error("No virtual interface with given index:[%s].", vif_index)
        return None

    def create_new_vif(self, inst_name, device_name, vif_index):
        """
    ...
    def destroy_vif(self, inst_name, vif_index):
        ...
        if vif_ref is None:
            log.debug("No vif found with index [%s] when try to destroy vif.", vif_index)
            return False
    ...
    def attach_vif_to_vm(self, inst_name, vif_index):
        ...
        vif_ref = self.get_vif_by_index(inst_name, vif_index)
        if vif_ref is None:
            log.debug("No vif found with index [%s] when try to attach vif.", vif_index)
            return False
    ...
    def detach_vif_from_vm(self, inst_name, vif_index):
        ...
        vif_ref = self.get_vif_by_index(inst_name, vif_index)
        if vif_ref is None:
            log.debug("No vif found with index [%s] when try to detach vif.", vif_index)
            return False
